...
if(chamber == 'A.csv')  {
  ...
  colnames(on) <- c('datetime_A_ON', 'airtemp_A_ON', 'RH_A_ON', 'soiltemp_A_ON', 'soilmoisture_A_ON', 'VWC_A_ON','AH_A_ON')
  colnames(off) <- c('datetime_A_OFF', 'airtemp_A_OFF', 'RH_A_OFF', 'soiltemp_A_OFF', 'soilmoisture_A_OFF', 'VWC_A_OFF','AH_A_OFF')
  ...
  colnames(on) <- c('datetime_B_ON', 'airtemp_B_ON', 'RH_B_ON', 'soiltemp_B_ON', 'soilmoisture_B_ON', 'VWC_B_ON','AH_B_ON')
  colnames(off) <- c('datetime_B_OFF', 'airtemp_B_OFF', 'RH_B_OFF', 'soiltemp_B_OFF', 'soilmoisture_B_OFF', 'VWC_B_OFF','AH_B_OFF')
  ...
  colnames(on) <- c('datetime_C_ON', 'airtemp_C_ON', 'RH_C_ON', 'soiltemp_C_ON', 'soilmoisture_C_ON', 'VWC_C_ON','AH_C_ON')
  colnames(off) <- c('datetime_C_OFF', 'airtemp_C_OFF', 'RH_C_OFF', 'soiltemp_C_OFF', 'soilmoisture_C_OFF', 'VWC_C_OFF', 'AH_C_OFF')
} else if (chamber == 'D.csv')  {
  print('Subsetting for chamber D')
  on <- subset(csv, csv$datetime$min >= 45)
  off <- subset(csv, csv$datetime$min < 45)
  colnames(on) <- c('datetime_D_ON', 'airtemp_D_ON', 'RH_D_ON', 'soiltemp_D_ON', 'soilmoisture_D_ON', 'VWC_D_ON', 'AH_D_ON')
  colnames(off) <- c('datetime_D_OFF', 'airtemp_D_OFF', 'RH_D_OFF', 'soiltemp_D_OFF', 'soilmoisture_D_OFF', 'VWC_D_OFF', 'AH_D_OFF')
} else  {
