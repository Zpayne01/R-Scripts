...
{ no2a[i] <- (blc2acounts[i]-blc2acounts[i]*ceHONO2)/(ceNO2*c)
  no2asd[i] <- blc2acountssd/(c*ceNO2)
} else
...
if (honob[i] > 0) 
  { no2b[i] <- (blc2bcounts[i] - blc2bcounts[i] * ceHONO2)/(ceNO2 * c)
    no2bsd[i] <- blc2bcountssd/(c*ceNO2)
} else 
...
no2csd[i] <- blc2ccountssd/(c*ceNO2)
} else 
...
if (honod[i] > 0)  { 
  ...
  no2dsd[i] <- blc2dcountssd/(c*ceNO2)
} else 
...
no2ambsd[i] <- blc2ambcountssd/(c*ceNO2)
} else 
