...
f <- list.files('D:/Surface 3/Main Folder/Documents/Lab Data/2019_11 Soil Incubation Study/LICOR Log')
...
file <- paste0('D:/Surface 3/Main Folder/Documents/Lab Data/2019_11 Soil Incubation Study/LICOR Log/',f[i])
...
datetime <- strptime(parse_date_time(datetime, orders = c('%Y-%m-%d %H:%M:%S', '%m/%d/%Y %H:%M:%S')), format = '%Y-%m-%d %H:%M:%S') #strptime((datetime), format='%Y-%m-%d %H:%M:%S', tz = 'EST')
...
co2FluxBsd <- sdco2b$CO2_ppm*(1/10^6)*(1/24.05)*(12/1)*(10^6/1)*(flowb)*(60)*(1/massb)
co2FluxCsd <- sdco2c$CO2_ppm*(1/10^6)*(1/24.05)*(12/1)*(10^6/1)*(flowc)*(60)*(1/massc)
co2FluxDsd <- sdco2d$CO2_ppm*(1/10^6)*(1/24.05)*(12/1)*(10^6/1)*(flowd)*(60)*(1/massd)
co2FluxEsd <- sdco2e$CO2_ppm*(1/10^6)*(1/24.05)*(12/1)*(10^6/1)*(flowe)*(60)*(1/masse)

co2FluxB[co2FluxB < -.5 | co2FluxB > 30] <- 'NA'  
co2FluxC[co2FluxC < -.5 | co2FluxC > 30] <- 'NA'  
co2FluxD[co2FluxD < -.5 | co2FluxD > 30] <- 'NA'  
co2FluxE[co2FluxE < -.5 | co2FluxE > 30] <- 'NA'  
...
png(paste0('D:/Surface 3/Main Folder/Documents/Lab Data/2019_11 Soil Incubation Study/CO2 Flux Plots/',filedate,'.png'))
...
## Convert water in ppth to absolute humidity (g m^-3) (at STP)
...
png(paste0('D:/Surface 3/Main Folder/Documents/Lab Data/2019_11 Soil Incubation Study/Absolute Humidity Plots/',filedate,'.png'))
...
## Combine into a single exportable file

co2FluxComb <- data.frame(seq(1:length(co2FluxB)),co2FluxB, co2FluxBsd, co2FluxC, co2FluxCsd, co2FluxD, co2FluxDsd, co2FluxE, co2FluxEsd)

export <- co2FluxComb
colnames(export) <- c("hour", "CO2_ug_C_g-1_soil_hr-1_B", "B_sd", "CO2_ug_C_g-1_soil_hr-1_C", "C_sd", "CO2_ug_C_g-1_soil_hr-1_D", "D_sd", "CO2_ug_C_g-1_soil_hr-1_E", "E_sd")

filedate <- gsub('-','_',as.Date(datetime[1]))
filename <- paste0("C:/Users/zacpayne/Desktop/Working CO2/",filedate," CO2.csv")
filename2 <- paste0("C:/Users/zacpayne/Desktop/Working H2O/",filedate," H2O.csv")
