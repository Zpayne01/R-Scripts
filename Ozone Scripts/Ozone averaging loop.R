...
for (i in 1:length(f))
	{ csv <- paste0(f[i],"/",list.files(f[i], pattern = "Ozone_amb_2"))
	  ozone <- read.csv(csv, header = TRUE)

colnames(ozone) <- c('ozone', 'datetime')
...
filename <- "Ozone_Stats_amb.csv"
write.table(stats, file = paste0(f[i] ,"/", filename), append = FALSE, sep = ",", row.names = FALSE)

csv <- paste0(f[i], "/", list.files(f[i], pattern = "Ozone_Chambers.csv"))
ozone <- read.csv(csv, header = TRUE)
...
filename <- "Ozone_Stats_chambers.csv"
write.table(stats, file = paste0(f[i],"/",filename), append = FALSE, sep = ",", row.names = FALSE)
}
